...
# DEM 边界安全边距（栅格数）
# ========================================
BOUNDARY_SAFE_MARGIN_CELLS = 2  # 距离 DEM 边界至少 2 个栅格
...
def choose_local_goal_index(
    ...
    if (inside_grid(final_col, final_row, grid_size, grid_size) and 
        ...
        print(f"【终点选择】选择最终终点: 索引={final_idx}, 栅格=({final_col}, {final_row}), "
    ...
    for idx in range(current_global_idx + 1, max_check_idx + 1):
        ...
        # 满足条件，记录（继续循环找更远的/序列号更大的）
        best_idx = idx
        best_col = col
        best_row = row
    
    if best_idx is not None:
        print(f"【终点选择】选择局部目标: 索引={best_idx}, 栅格=({best_col}, {best_row}), "
              f"距离边界 >= {BOUNDARY_SAFE_MARGIN_CELLS} 栅格")
...
def compute_path_length_world(path_world):
    ...
    for i in range(1, len(path_world)):
        length += calculate_distance(path_world[i - 1][0], path_world[i - 1][1], path_world[i][0], path_world[i][1])
    return length
...
def compute_future_benefit(path_world, original_goal_idx, waypoints_with_yaw,
                           dem_range=LOCAL_DEM_RANGE, resolution=LOCAL_DEM_RESOLUTION):
    """
    计算未来收益（奖励项）
    到达新目标点后，新 DEM 能覆盖到的"原终点之后"的全局路径点数量
    即索引严格大于原终点索引的、落在当前 DEM 范围内的全局路径点
    
    注意：这里使用候选路径的终点位置和朝向来计算新的 DEM 覆盖范围
    """
    grid_size = int((dem_range * 2) / resolution)
    count = 0
    ...
    # 计算候选路径终点处的朝向
    if len(path_world) >= 2:
        yaw = calculate_yaw_between_points(path_world[-2][0], path_world[-2][1], 
    ...
    # 只统计原终点索引之后的点（严格大于 original_goal_idx）
...
def score_candidates(candidate_results, waypoints_with_yaw, current_global_idx, original_goal_idx):
    ...
    for item in candidate_results:
        ...
        metric = {
            "item": item,
            # 原始指标
            "d_goal": calculate_distance(candidate_goal_world[0], candidate_goal_world[1],
    ...
    # 计算最终评分（越高越好）
    # J = w6*S_future - w1*d_goal - w2*d_path_dev - w3*L_path - w4*C_clearance - w5*C_heading - w7*C_turn
    for metric in metrics:
        reward = W_FUTURE * metric["norm_S_future"]
        penalty = (W_D_GOAL * metric["norm_d_goal"] +
